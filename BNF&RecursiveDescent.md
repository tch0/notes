...
ABNF使用空白符而不是逗号来串联，所以与选择运算符在一起时会混淆，需要用`()`来明确优先级。`(a b) / (c d)`
...
总结：其实EBNF和ABNF规则都大同小异，侧重点不同罢了，ABNF更侧重于网络传输，所以将字符编码作为最基本的非终结符，确保二进制位的正确性。组合选择的优先级也不太一样，需要注意。
