...
现在我有一堆问题，如果远程没有分支，本地有，但是分支不推送到远程，合并到master之后推送到远程的话远程仓库中是有本地分支的提交记录的，但是没有分支。因为合并之后本地master最新提交的父节点作为以来肯定是需要提交上去的。这样在远端就会看到有分叉但是没有分支。所以这时候就是在本地rebase之后再提交的用武之地吗？


还有rebase之后master到底移动还是不移动啊，待试验。
